    ...
    with open(name, 'r') as f:
        ...
        for pos in soup.find_all("section", {"class": "gramb"}):
            ...
            if lst:
                ...
                total_senses += 1
        if total_senses == 0:  # In case there was some error on no response, try to re-download the page and store them
            if name.split("/")[-1][:-5] + ".html" in listdir('google_v2'):
                continue
            r = requests.get(
                url="https://www.lexico.com/en/definition/" + name.split("/")[-1][:-5])
            time.sleep(5)
            soup = BeautifulSoup(r.content, 'lxml')
            try:
                new_name = soup.find_all('ul', {'class': "search-results unpadded"})[
                    0].findChildren('li', recursive=False)[0].text
                r = requests.get(
                    url="https://www.lexico.com/en/definition/" + new_name)
                time.sleep(5)
